...
```
rainfall_2$date is char  type
and temp_3$date is date type 

```{r}
combined_both_temps_rainfall <- merge(rainfall_2, temp_2, by = c("city_name", "date"))
head(combino_bambino)
```

### Remove this following chunk now?
Might not need this anymore now that the Date format is figured out
```{r}
