...
```{r}
modis_firedata <- readr::read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2020/2020-01-07/MODIS_C6_Australia_and_New_Zealand_7d.csv')
...
combined$date_2 <- as.Date(paste(combined$year,"-",combined$month,"-",1, sep = ""))

combined_wider <- pivot_wider(data=combined, names_from = temp_type, values_from = temperature)

combined_avg_rt <- aggregate(. ~city_name+date_2,data = combined_wider[, c("rainfall","max","min","city_name","date_2")], mean)

p1<-ggplot(data = combined_avg_rt) +
  geom_point(mapping = aes(y = rainfall, x = date_2),size=0.5, colour="blue")+
  geom_point(mapping = aes(y=min/1.5, x = date_2),size=0.001,alpha=0.75, colour="red") +
  geom_point(mapping = aes(y=max/1.5, x = date_2),size=0.001,alpha=0.75, colour="black") +
...
___Max and Min made into their own columns with temperature values___
```{r}
library(ggplot2)
library(dplyr)
library(patchwork)
...
combined_avg_rt <- aggregate(. ~city_name+year,data = combined_wider[, c("rainfall","max","min","city_name","year")], mean)

p1<-ggplot(data = combined_avg_rt) +
  geom_point(mapping = aes(y = rainfall, x = year),size=0.5, colour="blue")+
  geom_point(mapping = aes(y=min/1.5, x = year),size=0.001,alpha=0.75, colour="red") +
  geom_point(mapping = aes(y=max/1.5, x = year),size=0.001,alpha=0.75, colour="black") +
  scale_y_continuous(name = ("Rainfall (blue)"), sec.axis = sec_axis(~.*1.5,name="Temperature (Max=black, Min=red)"))+
  facet_grid(rows = vars(city_name))+
...
#res <- var.test(rainfall ~ year, data = syndey_only) #doesn't work because factors have more than 2 levles
#res
```
